#this version includes a baseline interaction parameter, h0, and fixes omega=sigma1
library(nimble)
library(coda)
nimbleOptions(determinePredictiveNodesInModel = FALSE)
source("sim.SCR.2species.softcore.R")
source("init.data.2species.softcore.R")
source("NimbleModel 2species Softcore Baseline shareSig.R")
...
omega <- sigma1 #repulsion scale parameter, assuming it is the same as sigma1 here
...
Niminits <- list(z1=nimbuild$z1,N1=nimbuild$N1, #must init N to be sum(z.init)
                 ...
                 D02=sum(nimbuild$z2)/(sum(data$constants$InSS)*data$constants$res^2),D.beta12=0,
                 sigma2=inits$sigma2,
                 p02=0.25,h0=0.5)
...
parameters <- c('p01','D.beta11','sigma1','N1','D01','lambda1',
              'p02','D.beta12','h0','sigma2','N2','D02','lambda2')
...
conf$addSampler(target = c("D02","D.beta12",'h0'),
                type = 'AF_slice',control=list(adaptive=TRUE),silent = TRUE)
...
#not sure about the cost of this since sigma1 now has species 2 dependencies...
